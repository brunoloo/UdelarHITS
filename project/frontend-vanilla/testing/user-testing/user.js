  ...
  if (!categories || categories.length === 0) {
    ...
    categories.forEach(c => {
      ...
      tr.innerHTML = `
        ...
        <td><a href="/testing/category-testing/category.html?id=${encodeURIComponent(c.id)}">${escapeHtml(c.titulo)}</a></td>
  ...
  if (!repliesResult.ok || repliesResult.data.length === 0) {
    ...
    repliesResult.data.forEach(r => {
      ...
      const destinoLink = r.tipo === 'tema'
        ? `<a href="/testing/topic-testing/topic.html?id=${encodeURIComponent(r.destino_id)}">${escapeHtml(r.destino_titulo)}</a>`
        : `<a href="/testing/category-testing/category.html?id=${encodeURIComponent(r.destino_id)}">${escapeHtml(r.destino_titulo)}</a>`;
      tr.innerHTML = `
  ...
  if (!followers || followers.length === 0) {
    ...
    followers.forEach(f => {
      ...
      tr.innerHTML = `
        ...
        <td><a href="/testing/user-testing/user.html?nickname=${encodeURIComponent(f.nickname)}">${escapeHtml(f.nickname)}</a></td>
