...
export const createVerification = async ({ email, codigo, nickname, nombre, passwordHash, facultad = null, expiraEn }) => {
  await pool.query(
  ...
  const q = `
    INSERT INTO verificacion_registro (email, codigo, nickname, nombre, password_hash, facultad, expira_en)
    VALUES ($1, $2, $3, $4, $5, $6, $7)
    RETURNING id, email, expira_en
  `;
  const { rows } = await pool.query(q, [email, codigo, nickname, nombre, passwordHash, facultad, expiraEn]);
  return rows[0];
};
...
export const findValidVerification = async (email) => {
  const q = `
    SELECT id, email, codigo, nickname, nombre, password_hash, facultad, intentos, expira_en
