async function loadCategory() {
  ...
  categoryBody.innerHTML = `
    ...
    <tr><th>Autor</th><td><a href="/testing/user-testing/user.html?nickname=${encodeURIComponent(c.autor_nickname)}">${escapeHtml(c.autor_nickname)}</a></td></tr>
  ...
  if (!c.topics || c.topics.length === 0) {
    ...
    c.topics.forEach(t => {
      ...
      tr.innerHTML = `
        ...
        <td><a href="/testing/topic-testing/topic.html?id=${encodeURIComponent(t.contenido_id)}">${escapeHtml(t.titulo)}</a></td>
  ...
  if (!repliesResult.ok || repliesResult.data.length === 0) {
    ...
    repliesResult.data.forEach(r => {
      ...
      tr.innerHTML = `
        ...
        <td><a href="/testing/user-testing/user.html?nickname=${encodeURIComponent(r.autor_nickname)}">${escapeHtml(r.autor_nickname)}</a></td>
